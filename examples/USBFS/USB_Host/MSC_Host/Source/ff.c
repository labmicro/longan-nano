...
static void unlock_fs(FATFS * fs, /* File system object */
    ...
    if (res != FR_NOT_ENABLED && res != FR_INVALID_DRIVE && res != FR_INVALID_OBJECT &&
        res != FR_TIMEOUT) {
...
static FRESULT dir_seek(DIR * dj, /* Pointer to directory object */
    ...
    if (clst == 1 || clst >= dj->fs->max_clust) /* Check start cluster range */
        return FR_INT_ERR;
    if (!clst &&
        dj->fs->fs_type == FS_FAT32) /* Replace cluster# 0 with root cluster# if in FAT32 */
        clst = dj->fs->dirbase;
        ...
        if (c == 0xE5 || ((a & AM_VOL) && a != AM_LFN)) { /* An entry without valid data */
            ...
            if (a == AM_LFN) { /* An LFN entry is found */
                if (dj->lfn) {
                    ...
                    ord = (c == ord && sum == dir[LDIR_Chksum] && cmp_lfn(dj->lfn, dir)) ? ord - 1
                                                                                         : 0xFF;
                }
    ...
    while (dj->sect) {
        ...
        if (c == 0xE5 || (!_FS_RPATH && c == '.') ||
            ...
            if (a == AM_LFN) {  /* An LFN entry is found */
                ...
                ord = (c == ord && sum == dir[LDIR_Chksum] && pick_lfn(dj->lfn, dir)) ? ord - 1
                                                                                      : 0xFF;
            } else {                            /* An SFN entry is found */
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
static FRESULT dir_register(/* FR_OK:Successful, FR_DENIED:No free entry or too many SFN collision,
...
static FRESULT dir_remove(         /* FR_OK: Successful, FR_DISK_ERR: A disk error */
    ...
    res = dir_seek(dj, (WORD)((dj->lfn_idx == 0xFFFF)
                                  ? i
...
static BYTE check_fs(/* 0:The FAT boot record, 1:Valid boot record but not an FAT, 2:Not a boot
                        record, 3:Error */
                     FATFS * fs, /* File system object */
    ...
    if (disk_read(fs->drive, fs->win, sect, 1) != RES_OK) /* Load boot record */
        return 3;
    if (LD_WORD(&fs->win[BS_55AA]) != 0xAA55) /* Check record signature (always placed at offset 510
...
FRESULT chk_mounted(                     /* FR_OK(0): successful, !=0: any error occurred */
    ...
    *rfs = fs = FatFs[vol]; /* Return pointer to the corresponding file system object */
    ...
    fs->database =
        fs->fatbase + fsize + fs->n_rootdir / (SS(fs) / 32); /* Data start sector (lba) */
    ...
    if (fmt == FS_FAT32) {
        ...
        if (disk_read(fs->drive, fs->win, fs->fsi_sector, 1) == RES_OK &&
            LD_WORD(fs->win + BS_55AA) == 0xAA55 && LD_DWORD(fs->win + FSI_LeadSig) == 0x41615252 &&
...
FRESULT f_open(FIL * fp,           /* Pointer to the blank file object */
    ...
    res =
        chk_mounted(&path, &dj.fs,
                    (BYTE)(mode & (FA_WRITE | FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)));
#else
    mode &= FA_READ;
...
FRESULT f_read(FIL * fp,    /* Pointer to the file object */
    ...
    *br = 0; /* Initialize bytes read */
    ...
    for (; btr; /* Repeat until all data transferred */
...
#if _FS_TINY
                if (fp->fs->wflag &&
                    fp->fs->winsect - sect < cc) /* Replace one of the read sectors with cached data
                                                    if it contains a dirty sector */
                    mem_cpy(rbuff + ((fp->fs->winsect - sect) * SS(fp->fs)), fp->fs->win,
                            SS(fp->fs));
#else
                if ((fp->flag & FA__DIRTY) &&
                    fp->dsect - sect < cc) /* Replace one of the read sectors with cached data if it
        ...
        rcnt =
            SS(fp->fs) - (fp->fptr % SS(fp->fs)); /* Get partial sector data from sector buffer */
        if (rcnt > btr)
            rcnt = btr;
#if _FS_TINY
        ...
        if ((fp->fptr % SS(fp->fs)) == 0) {   /* On the sector boundary? */
            if (fp->csect >= fp->fs->csize) { /* On the cluster boundary? */
                if (fp->fptr == 0) {          /* On the top of the file? */
                    ...
                    clst =
                        create_chain(fp->fs, fp->curr_clust); /* Follow or stretch cluster chain */
                }
                if (clst == 0)
...
#else
            if (fp->flag &
                FA__DIRTY) { /* Write back data buffer prior to following direct transfer */
                if (disk_write(fp->fs->drive, fp->buf, fp->dsect, 1) != RES_OK)
                    ABORT(fp->fs, FR_DISK_ERR);
                fp->flag &= ~FA__DIRTY;
            }
...
FRESULT
f_getfree(const XCHAR * path, /* Pointer to the logical drive number (root dir) */
    ...
    if (res == FR_OK) {
        ...
        if (res == FR_OK) {
            ...
            if (!dir) { /* Is it a root directory? */
                ...
                dir[DIR_Attr] =
                    (value & mask) | (dir[DIR_Attr] & (BYTE)~mask); /* Apply attribute change */
                dj.fs->wflag = 1;
                res = sync(dj.fs);
            }
        }
        ...
        if (res == FR_OK) {
            ...
            if (dir[DIR_Attr] & AM_DIR) { /* Update .. entry in the directory if needed */
                dw = clust2sect(dj_new.fs,
                                (DWORD)LD_WORD(dir + DIR_FstClusHI) | LD_WORD(dir + DIR_FstClusLO));
                if (!dw) {
                    res = FR_INT_ERR;
                } else {
                    ...
                    if (res == FR_OK && dir[1] == '.') {
                        dw = (dj_new.fs->fs_type == FS_FAT32 && dj_new.sclust == dj_new.fs->dirbase)
                                 ? 0
                                 : dj_new.sclust;
...
FRESULT f_mkfs(BYTE drv,       /* Logical drive number */
    ...
    static const DWORD sstbl[] = {2048000, 1024000, 512000, 256000, 128000, 64000,
                                  32000,   16000,   8000,   4000,   0};
    static const WORD cstbl[] = {32768, 16384, 8192, 4096, 2048, 16384,
